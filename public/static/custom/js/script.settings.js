    ...
    $.ajax({
        ...
        success : function(data){
            if($.isEmptyObject(data)){
                table.html('<tr><td colspan="99">Không tìm thấy</td></tr>');
            } else {
                table.html('');
...
$(document).ready(function(){
    ...
    $(document).on("click","#addStudentCategory",function(){
        ...
        if(category == "" || max_allowed == "" ){
            module_body.prepend(templates.alert_box( {type: 'danger', message: 'Yêu cầu nhập đầy đủ'} ));
            send_flag = false;
        }
    ...
    $(document).on("click","#addBranch",function(){
        ...
        if(branch == ""  ){
            module_body.prepend(templates.alert_box( {type: 'danger', message: 'Yêu cầu chọn Khoa'} ));
            send_flag = false;
        }
