...
class LogController extends Controller
{
    public function index()
	{
		...
		for($i=0; $i<count($logs); $i++){
			...
			if ($issue->return_time == 0) {
				$logs[$i]['return_time'] =  '<p class="color:red">Đang Mượn</p>';
			}else {
	...
	public function store(Request $request)
	{
		...
		if($student == NULL){
			...
			if($approved == 0){
				...
				return "Sinh Viên chưa được duyệt !";
				// throw new Exception('');
			} else {
				...
				if($books_issued >= $max_allowed){
					...
					return 'Sinh Viên không thể mượn thêm !';
				} else {
	...
	public function edit($id)
	{
		...
		if(!$log->count()){
			return 'ID không hợp lệ hoặc Sách đã trả !!!';
		} else {
