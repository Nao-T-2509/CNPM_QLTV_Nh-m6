...
class StudentController extends Controller
{
	...
	public function postRegistration(Request $request){
		...
		if(!$validator) {
			...
			if($student){
				return Redirect::route('student-registration')
					->with('global', 'Yêu cầu của bạn đã thực hiện, vui lòng chờ trong ít phút !');
			}
		}
	}
	...
	public function StoreSetting(Request $request)
	{
		...
		if ($request->category) {
			...
			if (!$student) {
				return "Thêm Khóa không thành công!";
			}else {
				return "Thêm Khóa thành công!";
				// return back();
			}
		}elseif ($request->branch) {
			...
			if (!$branch) {
				return "Thêm Khoa không thành công!";
			}else {
